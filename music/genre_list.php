...
<div class="col-lg-12">
	<div class="d-flex justify-content-between align-items-center w-100">
		<div class="form-group" style="width:calc(50%) ">
			<div class="input-group">
              <input type="search" id="filter" class="form-control form-control-sm" placeholder="Search Genre using keyword" style="font-family: 'Poppins';background: #121212 !important;color: white;">
	...
	<div class="row" id="genre-list" style="font-family: 'Poppins' !important;">
		<?php 
			$genres= $conn->query("SELECT * FROM genres order by genre asc");
			while($row=$genres->fetch_assoc()):
		?>
		<div class="card bg-black genre-item my-2 mx-1 card-hover" data-id="<?php echo $row['id'] ?>" style="width:14.5vw;cursor:pointer;background: #1d1d1d;border-radius: 10px;padding-top: 5px;box-shadow: 0px 3px 3px 0px rgba(0, 0, 0, 0.57);">
			...
			<div class="card-img-top flex-w-100 position-relative py-2 px-3">
				...
				<img src="assets/uploads/<?php echo $row['cover_photo'] ?>" class="card-img-top"  style="object-fit: cover;max-width: 100%;height:26vh;border-radius:4px;" alt="Genre Cover">
			</div>
			<div class="card-body" style="height: 20vh">
				<div class="card-title"  style="color: white;margin-bottom: 10px;"><?php echo ucwords($row['genre']) ?></div>
				<p class="card-text truncate text-white" style="color:#9c9c9c!important"><?php echo $row['description'] ?></p>
			</div>
			</a>
		</div>
		<?php endwhile; ?>
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp == 1) {
                ...
                $('.modal').modal('hide');
                location.href = document.URL; // Reload the page
                end_load();
            }
        }
    });
